...
void goBack(){ 
      digitalWrite(light_BR, HIGH); digitalWrite(light_BR, HIGH);
...
if (parkingLight) {digitalWrite(parking_Light, HIGH);}
if (!parkingLight) {digitalWrite(parking_Light, LOW);}
switch (command) {
...
case 'X':parkingLight = true;break;
case 'x':parkingLight = false;break;
}
}
}
